...
\sigma_{k_i}^2 &\overset{iid}\sim \text{Inv}\chi^2_2
\end{align*}
...
&\ \ \ \ \ \ \ \ \times \exp\Big\{-\frac{1}{2}\mu_{k_i}^T\mu_{k_i}\Big\}\exp\Big\{-\frac{1}{2}\mu_{k_j}^T\mu_{k_j}\Big\}  \times \frac{1}{(\sigma_{k_i}^2)^2}\exp\Big\{-\frac{1}{2\sigma_{k_i}^2}\Big\}\frac{1}{(\sigma_{k_j}^2)^2}\exp\Big\{-\frac{1}{2\sigma_{k_j}^2}\Big\} \times \lambda_i \times \lambda_j
\end{align*}

Next, we turn to finding the full conditioal of each parameter in this likelihood function. At each iteration of the Gibbs sampler, $t$, we conditional on the current parmater vector $\theta^{(t)}$ and sample each parameter in stepwise fashion. We write the full conditionals as follows. 
...
\end{align*}
We can complete an identical analysis for $\mu_{k_j}$ and we arrive at the following deistributions. 
\begin{align*}
...
\begin{align*}
f_{\sigma_{k_i}^2|\theta, Y}(\sigma_{k_i}^2|\theta, Y) &\propto \frac{1}{2\sigma_{k_i}}\exp\left\{-\frac{1}{2\sigma_{k_i}^2}(Z_i-\mu_{k_i})^T(Z_i-\mu_{k_i})\right\}\frac{1}{(\sigma_{k_i}^2)^2}\exp\left\{-\frac{1}{2\sigma_{k_i}^2}\right\}\\
&\propto \frac{1}{(\sigma_{k_i}^2)^{5/2}}\exp\left\{-\frac{1}{2\sigma_{k_i}^2}\left((Z_i-\mu_{k_i})^T(Z_i-\mu_{k_i})+1\right)\right\}
\end{align*}
