...
Instead, we simplify our model to make EM more analytically tractable. In Section \ref{MCMC}, we fit the full model using MCMC. Here, we fit the simplified model using latent distances $d_{ij} \equiv \Vert Z_i - Z_j \Vert$

\begin{align*}
Y_{ij} | Z_i, Z_j &\sim \text{Bern}\Big[\text{logit}^{-1}\big(d_{ij}\big)\Big] \\
...
&\ \ \ \ \ \ \ \ \times \frac{\Gamma(\alpha + \beta)}{\Gamma(\alpha)\Gamma(\beta)}d_{ij}^{\alpha - 1}(1-d_{ij})^{\beta - 1} \\
l(d_{ij}, \alpha, \beta ; Y) &= \sum_{i<j} Y_{ij}\log \Big(\frac{d_{ij}}{1 - d_{ij}}\Big) + (1-d_{ij})  \\
