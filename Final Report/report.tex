...
\begin{figure}\label{filtered_net}
	\centering
	\includegraphics[width = .45\textwidth]{initial_net.pdf}
	\includegraphics[width = .45\textwidth]{filtered_net.pdf}
\end{figure}
...
\begin{enumerate}
	\item Need to incorperate edges weights for better performance
	\item Heatmaps showing better fit that the unweighted model
\end{enumerate}
...
\begin{enumerate}
	\item We see that the density of the distances show bimodal behavior
	\item Spectral clustering detects nontrivial clusters in the network
\end{enumerate}
