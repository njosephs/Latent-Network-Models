...
\begin{align*}
\mathcal{L}(Z, \theta; Y) &= \prod_{i<j}\mathbb{P}(Y_{ij} | Z_i, Z_j) \mathbb{P}(Z_i | K_i, \mu_k, \sigma_k^2) \mathbb{P}(Z_j | K_j, \mu_j, \sigma_j^2) \mathbb{P}(K_i)\mathbb{P}(\mu_k)\mathbb{P}(\sigma_k^2)\mathbb{P}(K_j)\mathbb{P}(\mu_j)\mathbb{P}(\sigma_j^2) \\
&= \prod_{i<j}\Big(\text{logit}^{-1}\big(\Vert Z_i-Z_j\Vert)\Big)^{Y_{ij}}\Big(1 - \text{logit}^{-1}\big(\Vert Z_i-Z_j\Vert)\Big)^{1 - Y_{ij}} \\
&\ \ \ \ \ \ \ \ \times \phi(\mu_k, \sigma_k^2)\times \frac{1}{G} \times f_{MVN}(0, I_2) \times f_{\chi^2_2} \\
&\ \ \ \ \ \ \ \ \times \phi(\mu_j, \sigma_j^2)\times \frac{1}{G} \times f_{MVN}(0, I_2) \times f_{\chi^2_2}
\end{align*}
Hence the log-likelihood, substituting the forms of the given densities, is

\begin{align*}
l(Z, \theta ; Y) &= \sum_{i<j} \Big\{Y_{ij} \Vert Z_i - Z_j \Vert - \log \big(1 + \exp\big(\Vert Z_i - Z_j\Vert\big)\big)\Big\} \\
