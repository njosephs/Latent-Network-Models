...
where
\[Z_i \overset{iid}{\sim}\sum_{k=1}^G \lambda_g\text{MVN}_d(\mu_k,\sigma_k^2I_d)\]
...
\lambda_k &\overset{iid}\sim \frac{1}{G} \textbf{DECIDE IF WE NEED THIS} \\
\mu_k &\overset{iid}\sim \text{MVN}_2(0, I_2) \\
...
\end{align*}
where $p_{ij} \equiv \text{logit}^{-1} \frac{1}{\Vert Z_i - Z_j \Vert} = \text{logit}^{-1} \frac{1}{d_{ij}} $ are latent (standardized) distances. Then
...
\gamma_{ij} &\equiv \mathbb{E}_{p_{ij} | Y_{ij}, \theta} \big[\log (1 - p_{ij})\big] = \Psi\Big(\beta + 1 - Y_{ij}\Big) - \Psi\Big(\alpha + \beta + 1\Big)
\end{align*}
...
\begin{algorithm*}\label{EM_alg}
    ...
    \Output{Nuisance Parameters $\alpha^*$, $\beta^*$ \\ Latent Distance Estimates $\hat{d}$}
...
\begin{algorithm*}
    ...
    \For{$t = 2,\ldots,ns$} {
    \For{$i = 1, \dotsc, N_v$}{
  	sample $K_{k_i} \sim \text{Multinoulli}(G, \lambda)$\;
    }
    \For{$k = 1, \dotsc, N_k$}{
    	sample $\mu_{k}|\theta^{(t)},Y  \sim MVN_d\left(\sum_{i = 1}^{N_v}\mathbb{I}\{k_i = k\}\frac{Z_i^{(t)}}{(\sigma_{k_i}^2)^{(t)}+1}, \sum_{i = 1}^{N_v}\mathbb{I}\{k_i = k\}\frac{(\sigma_{k_i}^2)^{(t)}}{(\sigma_{k_i}^2)^{(t)}+1}I_2\right)$\;
    }
    \For{$k = 1, \dotsc, N_k$}{
    	sample $\sigma_{k}^2|\theta^{(t)},Y \sim \text{Inv}\Gamma\left(3/2, \frac{(Z_i - \mu_{k}^{(t)})^T(Z_i - \mu_{k}^{(t)})}{2} + 1\right)$ \textbf{REPLACE} \;
    }
   \For{$i=1, \dotsc, N_v$}{
