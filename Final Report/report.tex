...
\begin{align*}
\alpha^{(t+1)} &= \alpha^{(t)} - \frac{\Psi\big(\alpha^{(t)} + \beta^{(t)}\big) - \Psi(\alpha^{(t)}) + \sum_{i < j}\frac{ \mathbb{E}_{d_{ij} | Y_{ij}, \theta^{(t)}} \big[\log d_{ij}\big] }{{n \choose 2}}}{\Psi'\big(\alpha^{(t)} + \beta^{(t)}\big) - \Psi'(\alpha^{(t)})}
\end{align*}

\begin{align*}
\beta^{(t+1)} &= \beta^{(t)} - \frac{\Psi\big(\alpha^{(t+1)} + \beta^{(t)}\big) - \Psi(\beta^{(t)}) + \sum_{i < j}\frac{ \mathbb{E}_{d_{ij} | Y_{ij}, \theta^{(t)}} \big[\log (1- d_{ij})\big] }{{n \choose 2}}}{\Psi'\big(\alpha^{(t+1)} + \beta^{(t)}\big) - \Psi'(\beta^{(t)})}
\end{align*}
...
\begin{algorithm*}
    ...
    \underline{LNM EM} $(G)$\;
    \Input{Graph $G$-- do we want to input G or Y}
    \Output{Latent Distances $d_{ij}$-- what do we want here?}
    Initialize values $\alpha^{(0)}$, $\beta^{(0)}$\;
    Calculate $\pi^{(0)}_{\alpha}$, $\pi^{(t0}_{\beta}$\;
    \Repeat{Q converge}{
     update $\alpha^{(t+1)}$ using $\alpha^{(t)}$, $\beta^{(t)}$ and $\pi^{(t)}_{\alpha}$\;
     update $\beta^{(t+1)}$ using $\beta^{(t)}$, $\alpha^{(t+1)}$ and $\pi^{(t)}_{\beta}$\;
     calculate $\pi^{(t+1)}_{d}$, $\pi^{(t+1)}_{\alpha}$, $\pi^{(t+1)}_{\beta}$ using $\alpha^{(t+1)}$, $\beta^{(t+1)}$\;
     calculate $Q(\theta, \theta^{(t+1)})$ using $\pi^{(t+1)}_{d}$, $\pi^{(t+1)}_{\alpha}$, $\pi^{(t+1)}_{\beta}$, $\alpha^{(t+1)}$, $\beta^{(t+1)}$
     }
     \Return $\alpha^*$, $\beta^*$\;
     calculate $\pi_d$;
...
\begin{algorithm*}
    ...
    Initialize vectors for \textbf{params}\;
    Initialize \textbf{params}$^{(0)}$\;
    \For{$t = 2,\ldots,ns$} {
   sample ${\mathbf{\mu}_k}_i|\theta^{(t)},Y  \sim MVN\left(\frac{Z_i^{(t)}}{(\sigma_{k_i}^2)^{(t)}+1}, \frac{(\sigma_{k_i}^2)^{(t)}}{(\sigma_{k_i}^2)^{(t)}+1 I_2}\right)$\;
  sample $\mu_{k_j}|\theta^{(t)},Y \sim MVN \left(\frac{Z_j^{(t)}}{(\sigma_{k_j}^2)^{(t)}+1}, \frac{(\sigma_{k_j}^2)^{(t)}}{(\sigma_{k_j}^2)^{(t)}+1}I_2\right)$\;
   sample $\sigma_{k_i}^2|\theta^{(t)},Y \sim \text{Inv}\Gamma\left(3/2, \frac{(Z_i - \mu_{k_i}^{(t)})^T(Z_i - \mu_{k_i}^{(t)})}{2} + 1\right)$ \;
   sample $\sigma_{k_j}^2|\theta^{(t)}, Y \sim \text{Inv}\Gamma\left(3/2, \frac{(Z_j - \mu_{k_i}^{(t)})^T(Z_j - \mu_{k_j}^{(t)})}{2} + 1\right)$ \;
   sample $\mathbf{z}$;
   LOOK AT TIME POINTS FOR SIGMA;
   }
   Use parameters to sample Z?
